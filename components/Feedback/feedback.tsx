...
export const Feedback = ( { messageuuid, messageIndex }: { messageuuid?: string, messageIndex: number } ) => {
  ...
  const handleSubmitFeedback = () => {
    ...
    }).then(response => response.json()).then(data => alert(data.message)).catch(err => console.log('feedbackerr: ', err));
    // api call done
  ...
  return (
    <>
      ...
      {isClicked && (
        <>
          <div className="z-10 fixed inset-0 flex items-center justify-center bg-black bg-opacity-10">
            <div className="fixed inset-0 z-10 overflow-hidden">
              <div className="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <Draggable>
                  <div
                    ref={modalRef}
                    className="z-100 dark:border-netural-400 inline-block max-h-[400px] transform overflow-y-auto rounded-lg border border-gray-300 bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all dark:bg-[#202123] sm:my-8 sm:max-h-[600px] sm:w-full sm:max-w-lg sm:p-6 sm:align-middle"
                    role="dialog"
                  >
